...
MacCormack::MacCormack(int imax, int jmax, bool axylsymmetric) {
	for (int k = 0; k < 4; k++) {
	...
	for (int i = 1; i < imax - 1; i++) {
		for (int j = 1; j < jmax - 1; j++) if (type.Get(i, j) == INSIDE) {
			for (int k = 0; k < 4; k++) {
				...
				double delta = -delta_t / delta_x * (E_[k].Get(i + 1, j) - E_[k].Get(i, j))
					- delta_t / delta_y * (F_[k].Get(i, j + 1) - F_[k].Get(i, j));

				if (axilsymmetric_) {
					delta = -delta_t / delta_x * (E_[k].Get(i + 1, j) - E_[k].Get(i, j))
						- delta_t / delta_y / r * ((r + delta_y)*F_[k].Get(i, j + 1) - r * F_[k].Get(i, j)) +
						delta_t * H_[k].Get(i, j) / r;
				}

				U_predicted_[k].Set(i, j, U_[k].Get(i, j) + delta);
			}
		}
	}
	...
	for (int i = 1; i < imax - 1; i++) {
		for (int j = 1; j < jmax - 1; j++) if (type.Get(i, j) == INSIDE) {
			for (int k = 0; k < 4; k++) {
				...
				double delta = -delta_t / delta_x * (E_[k].Get(i, j) - E_[k].Get(i - 1, j))
					- delta_t / delta_y * (F_[k].Get(i, j) - F_[k].Get(i, j - 1));

				if (axilsymmetric_) {
					delta = -delta_t / delta_x * (E_[k].Get(i, j) - E_[k].Get(i - 1, j))
						- delta_t / delta_y / r * (r*F_[k].Get(i, j) - (r - delta_y)*F_[k].Get(i, j - 1)) +
						delta_t * H_[k].Get(i, j) / r;
				}

				U_corrected_[k].Set(i, j, 0.5*(U_corrected_[k].Get(i, j) + U_predicted_[k].Get(i, j) + delta));
			}
		}
	}
...
void MacCormack::CalcE(int imax, int jmax, double deltax, double deltay, const FlowParameters& params,
	...
	for (int i = 0; i < imax; i++) {
		for (int j = 0; j < jmax; j++) if (type.Get(i, j) != OUTSIDE)  {
...
void MacCormack::CalcH(int imax, int jmax, double deltax, double deltay, const FlowParameters& params,
	Array2D<double>& u, Array2D<double>& v, Array2D<double>& rho, Array2D<double>& P,
	Array2D<double>& T, Array2D<double>& e, Array2D<NODE_TYPE>& type, FiniteDifferencesType type_differences_x, FiniteDifferencesType type_differences_y) {
	...
	CalcAxilsymmetricStress(imax, jmax, deltax, deltay, params, u, v, T, type, type_differences_x, type_differences_y);

	for (int i = 0; i < imax; i++) {
		for (int j = 1; j < jmax; j++) if (type.Get(i, j) != OUTSIDE) {
	...
	for (int i = 0; i < imax; i++) {
		for (int j = 0; j < jmax; j++) if (type.Get(i, j) != OUTSIDE) {
			...
			if (!axilsymmetric_) {
				...
				tauxx_.Set(i, j, -2. / 3 * mu_ij * (ux_ij + vy_ij + v_ij / r_ij) + 2 * mu_ij * ux_ij);
				tauyy_.Set(i, j, -2. / 3 * mu_ij * (ux_ij + vy_ij + v_ij / r_ij) + 2 * mu_ij * vy_ij);
				tauxy_.Set(i, j, mu_ij * (uy_ij + vx_ij));
			}
		}
	}
...
void MacCormack::CalcAxilsymmetricStress(int imax, int jmax, double deltax, double deltay, const FlowParameters& params,
	Array2D<double>& u, Array2D<double>& v, Array2D<double>& T, Array2D<NODE_TYPE>& type,
	FiniteDifferencesType type_differences_x, FiniteDifferencesType type_differences_y) {
	...
	for (int i = 0; i < imax; i++) {
		for (int j = 1; j < jmax; j++) if (type.Get(i, j) != OUTSIDE) {
			...
			tauh_.Set(i, j, -2. / 3 * mu_ij * (ux_ij + vy_ij + v_ij / r_ij) + 2 * mu_ij * v_ij / r_ij);
		}
	}
